...
// On‑calc: no filesystem timestamps, no stat(), no time(), no struct tm.
...
// No format_date()
// No format_time()
// Both are deleted for CE build.

void init_macros(void) {
    // Minimal predefined macros for a freestanding TI‑84+ CE compiler.
    ...
    // Target type sizes for eZ80 ADL mode
    define_macro("__SIZEOF_SHORT__", "2");
    define_macro("__SIZEOF_INT__", "2");      // 16‑bit int
    define_macro("__SIZEOF_LONG__", "3");     // 24‑bit long
    define_macro("__SIZEOF_LONG_LONG__", "8");
    define_macro("__SIZEOF_POINTER__", "3");  // 24‑bit pointers
    define_macro("__SIZEOF_SIZE_T__", "3");
